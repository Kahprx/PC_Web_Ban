  ...
  useEffect(() => {
    const loadStats = async () => {
      ...
      try {
        ...
        notifyError(error, "Không tải được thống kê");
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="admin-page">
      <section className="admin-hero">
        <div className="admin-hero-header">
          <div>
            ...
            <h1>Dashboard thống kê theo API that.</h1>
      ...
      <section className="admin-overview-grid">
        <article className="admin-overview-card">
          ...
          <span>So đơn hàng toàn hệ thống.</span>
        </article>
        <article className="admin-overview-card">
          <p>Doanh thu</p>
          <strong>{formatVnd(overview?.total_revenue ?? 0)}</strong>
          <span>Tổng doanh thu từ bảng orders.</span>
        </article>
        <article className="admin-overview-card">
          <p>Pending</p>
          <strong>{overview?.pending_orders ?? 0}</strong>
          <span>Don can xử lý.</span>
        </article>
        <article className="admin-overview-card">
          <p>Completed</p>
      ...
      <section className="admin-surface">
        ...
        {loading ? <p>Đang tai thống kê...</p> : null}
        {!loading && chartRows.length === 0 ? <p>Chưa co dữ liệu thống kê.</p> : null}
